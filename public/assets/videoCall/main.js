...
function onLocalStream(stream) {
	...
	if($('#myvideo').length === 0)
		$('#videoleft').append('<video class="rounded centered" id="myvideo" width=100% height=100% controls autoplay playsinline muted="muted"/>');
...
function gotRemoteTrack(e) {
	if($('#remotevideo').length === 0) {
		addButtons = true;
		$('#videoright').append('<video class="rounded centered hide" id="remotevideo" controls width=100% height=100% autoplay playsinline/>');
...
function onSignalOffer(msg){
	...
	// Notify user
	//alert('You Have New Call from '+peerusername)


	$('#call').removeAttr('disabled').html('Call from '+peerusername+'  <span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span><span class="sr-only">Loading...</span>  <audio loop autoPlay><source src="/assets/sweet_text.mp3" type="audio/mpeg" /></audio>')
